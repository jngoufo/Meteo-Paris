            ...
            function askWeather(url, callback) {
                ...
                askWeather('https://www.prevision-meteo.ch/services/json/paris', function(jsonfile){
                    ...
                    let theParisHour = addZero(theTime.getUTCHours()+2); //Paris time is UTC+2
                    let theParisMin= addZero(theTime.getUTCMinutes());
                    ...
                    let nextHours = 23 - (theParisHour); // Hours left in the day.
                    function calculateNextHours() {
                        if (nextHours === -1) { // midnight in Paris	
                            nextHours = nextHours + 2;
                        } else if (nextHours === -2) { // 1am in Paris	
                            nextHours = nextHours + 4;
                        } else {
                            nextHours = theParisHour + 1; // paris time +1
                        }
                        return nextHours
                    } 
                    ...
                    let time, r1, p, r2, j;
                    if (nextHours===0) { 
                        nextHoursForecasts.remove();
                    }
                    else {  
                        nextHours = calculateNextHours();
                        for(time=23; time>=nextHours; time--) {
                            paris_time.push(time)// Let's put into the array, the hours remaining in the current day
                        }
                        paris_time.sort(function(a,b){return a-b});
                        let nextHoursH2 = document.createElement('H2');
                        nextHoursH2.innerHTML = "Plus tard aujourd'hui:";
                        nextHoursH2.className = 'nextHoursh2';
                        let todayForecastTable = createTable(2, paris_time.length);
                        todayForecastTable.className = 'todayforecasttable';                    
                        nextHoursForecasts.append(nextHoursH2, todayForecastTable);
                        for (r1=0, p=0; r1<document.querySelectorAll('.todayforecasttable tbody tr:nth-child(1)>td').length, p<paris_time.length; r1++, p++) {
                            document.querySelectorAll('.todayforecasttable tbody tr:nth-child(1)>td')[r1].innerHTML = '<span class="nexthourtemp">' +  paris_time[p] + '</span>' + 'H';
                        }; 
                        
                        for (r2=0, j=paris_time[0]; r2<document.querySelectorAll('table.todayforecasttable tbody tr:nth-child(2)>td').length, j<=23; r2++, j++){ // let display the next temperatures
                            document.querySelectorAll('table.todayforecasttable tbody tr:nth-child(2)>td')[r2].innerHTML = arrayOfMeteo[j][0].TMP2m + '<span class="celsius">' + '℃' + '</span>';//Note: arrayOfMeteo[1][0] = weather at 1am today; arrayOfMeteo[13][0] = weather at 13H today
                            ...
                            document.querySelectorAll('table.todayforecasttable tbody tr:nth-child(2)>td')[r2].insertAdjacentElement('beforeend',nextHoursIcon);
                        }
                    };
